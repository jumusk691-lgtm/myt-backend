import eventlet
# Sabse pehle monkey patch hona chahiye, kisi bhi aur import se pehle
...
import socketio
import time
# threading module ki ab zaroorat nahi kyunki hum eventlet.spawn use karenge
...
# async_mode='eventlet' bilkul sahi hai
...
def on_error(wsapp, error):
    global is_ws_ready
    is_ws_ready = False
    print(f"❌ WS Error: {error}. Retrying...")
    eventlet.sleep(5) # time.sleep ki jagah eventlet.sleep behtar hai
...
def login_to_angel():
    ...
    try:
        ...
        if session.get('status'):
            ...
            # WebSocket ko background mein chalane ke liye spawn karein
...
def subscribe(sid, data):
    ...
    # Wait for connection non-blocking way
    for _ in range(5):
...
# --- APP RUNNER ---
# Render/Gunicorn ke liye main entry point
