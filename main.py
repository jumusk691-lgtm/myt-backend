...
# Supabase Credentials (Logic: Service Role for Unlimited Storage Access)
...
# CRITICAL: 50MB Buffer size for handling massive watchlists and P2P stability
socketio = SocketIO(
    app, 
...
live_data_queue = {}               # High-speed buffer queue for data routing
last_tick_time = {}                # 1s Throttling tracker for Render stability
previous_price = {}                # History tracker for Red/Green APK logic
last_master_update_date = None

# ==============================================================================
# --- 3. MASTER DATA ENGINE (INDEXED DB LOGIC) ---
# ==============================================================================
def refresh_supabase_master():
    """
    Logic: Fetches 60,000+ symbols from Angel One, creates an indexed SQLite DB, 
    and uploads to Supabase so the APK can search symbols instantly.
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Optimization: Indexing for APK search speed (Required for smooth scrolling)
            ...
            # Uploading to Supabase Storage with Upsert
            ...
            print(f"⚠️ [System] Angel Master API Error: {response.status_code}")
            return False
...
# ==============================================================================
# --- 4. DATA BROADCASTER (P2P ROUTING LOGIC) ---
# ==============================================================================
def auto_batch_broadcaster():
    """
    Logic: This thread manages the distribution of ticks. 
    It implements P2P logic to reduce server load by nominating a 'Master' user.
    """
            ...
            for i in range(0, len(all_tokens), 300):
                ...
                for token in batch:
                    ...
                    if payload:
                        ...
                        # emit only to the Master SID of that token room.
                        if token in token_masters and token_masters[token]:
                            ...
                            # Fallback: Room broadcast if no specific master assigned
                            socketio.emit('live_update', payload, room=token)
                
                # Small micro-sleep to prevent socket congestion
                eventlet.sleep(0.02)
        else:
            eventlet.sleep(0.1)

# ==============================================================================
# --- 5. TICK ENGINE (ANGEL ONE RECEIVER LOGIC) ---
# ==============================================================================
def on_data(wsapp, msg):
    """
    Logic: Receives data from Angel WebSocket. 
    Implements 1s Throttling to keep the app and server stable.
    """
    ...
    try:
        if isinstance(msg, dict) and 'token' in msg:
            ...
            # Logic: Strictly 1 tick per second per token (Essential for Render stability)
            ...
            # Logic: Track previous price for APK Red/Green trend indicators
            old_p = previous_price.get(token, "{:.2f}".format(ltp))

            payload = {
                "t": token,
                "p": "{:.2f}".format(ltp),      # Current LTP
                "lp": old_p,                    # Last Price for trend calculation
        ...
        print(f"⚠️ [Tick Engine] Error: {e}")

# ==============================================================================
# --- 6. SELF-HEALING ENGINE (CONNECTION LOGIC) ---
# ==============================================================================
def run_trading_engine():
    """
    Logic: This is the core loop. It manages the Angel One connection, 
    market timing, and automatic session recovery.
    """
    ...
    while True:
        try:
            ...
            # Logic: Scheduled Master Update (8:35 AM IST daily)
            ...
            # Logic: Connectivity during Market Hours (including pre-market)
            if 7 <= now.hour < 24:
                if not is_ws_ready:
                    print("🔄 [System] Re-establishing Angel One Session...")
                    ...
                    if session.get('status'):
                        print("🟢 [System] Session Success. Launching WebSocket...")
                        ...
                        sws.on_data = on_data
                        sws.on_open = lambda ws: exec("global is_ws_ready; is_ws_ready=True; print('💎 WebSocket Engine Online!')")
                        sws.on_error = lambda ws, err: print(f"❌ [WS Error] {err}")
                        sws.on_close = lambda ws,c,r: exec("global is_ws_ready; is_ws_ready=False")
                        
                        sws.connect()
                    else:
                        print(f"❌ [Session Failure] {session.get('message')}")
            else:
                # Outside Market Hours Logic: Close WS to save Render RAM
                if is_ws_ready:
            ...
            print(f"🔴 [Critical Error] Re-looping: {e}")
            is_ws_ready = False
        
        eventlet.sleep(20)

# ==============================================================================
# --- 7. BATCHED SUBSCRIPTION & P2P ROUTING ---
# ==============================================================================
@socketio.on('subscribe')
def handle_subscribe(json_data):
    """
    Logic: Handles user watchlists. Implements 'Global Pooling'—if a token 
    is already subscribed by one user, it doesn't resubscribe to Angel, 
    but simply joins the room and potentially becomes a P2P Slave.
    """
    ...
    # Sorting into batches per exchange type
    batches = {1: [], 2: [], 3: [], 4: [], 5: []}
    ...
    for item in watchlist:
        ...
        # Standard Room Join for broadcasts
        join_room(token)

        # Logic: P2P Master Assignment
        ...
        # Logic: Signal newer users to connect to the Master for P2P data flow
        ...
        # Logic: Global Subscription Pooling (Subscription Batching)
    ...
    # Execution: Subscription in chunks of 50 (Angel One protocol limit)
            ...
            if tokens:
                for i in range(0, len(tokens), 50):
                    ...
                    try:
                        ...
                        print(f"📦 Subscribed {len(chunk)} tokens to Exchange {etype}")
                        eventlet.sleep(0.3)
                    except Exception as e:
                        print(f"⚠️ Subscription Chunk Error: {e}")
...
def forward_signal(data):
    """Logic: Relays WebRTC signals between Master and Slave nodes."""
...
def forward_ice(data):
    """Logic: Forwards ICE candidates for P2P connection establishment."""
    target = data.get('targetId')
    emit('ice_candidate', data, room=target)

@socketio.on('disconnect')
def on_disconnect():
    """Logic: Cleanup master lists on user exit."""
...
@app.route('/history')
def get_history():
    """Logic: Dynamic 90-day candle data fetch for APK charts."""
...
@app.route('/')
def health():
    """Logic: Real-time server diagnostics."""
    return {
        "engine": "READY" if is_ws_ready else "OFFLINE",
        "active_tokens": len(subscribed_tokens_set),
        "active_rooms": len(token_masters),
        "queue_load": len(live_data_queue),
        "market_time": datetime.datetime.now(IST).strftime('%H:%M:%S')
    }, 200
...
if __name__ == '__main__':
    ...
    print(f"🔥 [Server] TradeND Engine Booting on Port {port}...")
