...
# Client initialization ko safe rakhein
...
# Middleware to handle routing without causing AttributeError
...
def update_db_async(token, lp):
    try:
        # Supabase call ko background thread mein chalayein
...
def on_data(wsapp, msg):
    if isinstance(msg, dict) and 'last_traded_price' in msg:
        ...
        # DB update ko spawn karein taaki loop block na ho
        eventlet.spawn(update_db_async, token, lp)

# --- ANGEL LOGIN (Safe Execution) ---
def login_to_angel():
    global sws_instance
    try:
        obj = SmartConnect(api_key=API_KEY)
        totp = pyotp.TOTP(TOTP_KEY).now()
        # Login blocking call ho sakti hai, isliye ise separate thread mein rakha hai
        ...
        if session.get('status'):
            ...
            # WebSocket connection ko bhi spawn karein
...
def subscribe(sid, data):
    ...
    if data and sws_instance:
        ...
        # Subscribing call ko spawn karein
        eventlet.spawn(sws_instance.subscribe, "myt_pro", 3, subscription_list)
